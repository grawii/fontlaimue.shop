...
// เมื่อกดเปลี่ยน Tab ปุ่มล็อกอินจะสลับสีตาม Theme พรีเซ็ต ไม่ค้างเป็นสีขาวกระด้าง
...
function processUserAuth() {
    ...
    document.getElementById('unifiedAuthModal').classList.add('hidden'); updateCreditDisplay();
}
function logoutUser() { 
...
function openAllCategoriesPage() {
    hideAllPages(); document.getElementById('allCategoriesPage').classList.remove('hidden');
...
function openReviewPage() { 
    hideAllPages(); document.getElementById('reviewPage').classList.remove('hidden'); 
...
function renderReviewsList() {
    ...
    // แถบและปุ่มหมวดดาวในหน้ารีวิวรองรับตามธีมพื้นหลัง-กรอบ ไม่ค้างสีขาว
...
function submitNewReviewData() {
    const name = document.getElementById('revInputName').value.trim(); const text = document.getElementById('revInputText').value.trim();
...
function switchHistoryTab(tab) {
    ...
    document.getElementById('tabOrderBtn').className = tab === 'order' ? 'py-2.5 rounded-xl font-bold text-xs bg-white text-slate-800 border border-gray-300 flex-1' : 'py-2.5 rounded-xl font-bold text-xs bg-gray-100 text-gray-400 flex-1';
    document.getElementById('tabTopupBtn').className = tab === 'topup' ? 'py-2.5 rounded-xl font-bold text-xs bg-white text-slate-800 border border-gray-300 flex-1' : 'py-2.5 rounded-xl font-bold text-xs bg-gray-100 text-gray-400 flex-1';
    renderHistoryLogs();
}
...
function saveUserProfileData() {
    ...
    u.username = document.getElementById('usrEditName').value.trim();
    u.email = document.getElementById('usrEditEmail').value.trim();
    const np = document.getElementById('usrEditPass').value.trim(); if(np !== "") u.password = np;
...
/* ==========================================
   [ฟังก์ชันเพิ่ม] การจัดการ PROMOTION & TAXONOMY หลังบ้าน
   ========================================== */
...
function addNewPromoData() {
    const t = document.getElementById('addPromoTitle').value.trim();
    const i = document.getElementById('addPromoImg').value.trim();
    const l = document.getElementById('addPromoLink').value.trim();
    ...
    document.getElementById('addPromoTitle').value = ""; document.getElementById('addPromoImg').value = ""; document.getElementById('addPromoLink').value = "";
}
...
function addTaxonomyItem(field, inputId) {
    ...
    document.getElementById(inputId).value = ""; renderAdminDashboard();
}
