...
let isRegisterMode = false; // สลับโหมด ล็อกอิน / สมัครสมาชิก

let reviewsData = JSON.parse(localStorage.getItem('web_reviews')) || [
    { name: "คุณเอิร์น", score: 5, date: "14 พ.ค. 2567", text: "ฟอนต์สวยงาม ใช้งานง่าย ทางร้านบริการดีมากครับ", reply: "ขอบคุณที่ใช้บริการครับ" }
];
...
function updateCreditDisplay() {
    ...
    if(u) {
        // หากเข้าสู่ระบบแล้ว จะขึ้นรูปข้อความเครดิตสะสมคงเหลือ และคลิกเพื่อเปิดหน้ากระเป๋าเงินได้
        userArea.innerHTML = `
            <div onclick="openUserMenuPage()" class="cursor-pointer text-sub hover:underline">
                👤 บัญชี: <span class="text-main font-bold">${u.username}</span> | เครดิตสะสม: <span class="text-green-600 font-bold">฿${u.credit}</span>
            </div>`;
        const creditDetail = document.getElementById('userCreditDetail');
        if(creditDetail) creditDetail.innerText = u.credit;
    } else {
        userArea.innerHTML = `<button onclick="toggleUserAuthModal(true)" class="text-blue-500 hover:underline"><i class="fa-solid fa-right-to-bracket mr-1"></i>สมัครสมาชิก / เข้าสู่ระบบ</button>`;
    }
}
...
/* ==========================================
   2. MEMBER AUTHENTICATION (ระบบสมาชิกระบบแยกขาดกับแอดมิน)
   ========================================== */
function toggleUserAuthModal(s) { 
    document.getElementById('userAuthModal').classList.toggle('hidden', !s); 
    switchAuthMode(false); 
}

function switchAuthMode(mode) {
    isRegisterMode = mode;
    document.getElementById('authModalTitle').innerText = mode ? "สมัครสมาชิกใหม่ 🐰" : "เข้าสู่ระบบสมาชิก 🐰";
    document.getElementById('switchRegisterBtn').innerText = mode ? "มีบัญชีอยู่แล้ว? เข้าสู่ระบบ" : "ยังไม่มีบัญชี? สมัครสมาชิก";
    document.getElementById('switchRegisterBtn').onclick = () => switchAuthMode(!isRegisterMode);
}
...
function processUserAuth() {
    ...
    if(isRegisterMode) {
        // โหมดสมัครสมาชิก
        const exist = members.find(m => m.username === user);
        if(exist) return alert("ชื่อผู้ใช้งานนี้มีคนใช้ไปแล้วค่ะ");
        
        const newMem = { username: user, password: pass, credit: 0, orderHistory: [], topupHistory: [] };
        members.push(newMem);
        window.db.saveMembers(members);
        window.db.saveCurrentUser(newMem);
        alert("สมัครสมาชิกและเข้าสู่ระบบสำเร็จ! 🎉");
    } else {
        // โหมดเข้าสู่ระบบ
        const mem = members.find(m => m.username === user && m.password === pass);
        if(!mem) return alert("ชื่อผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง");
        window.db.saveCurrentUser(mem);
        alert(`ยินดีต้อนรับกลับมาค่ะ คุณ ${user} ✨`);
    }
...
function logoutUser() {
    ...
    updateCreditDisplay();
    alert("ออกจากระบบสมาชิกเรียบร้อย");
}

/* ==========================================
   3. STOREFRONT & FILTER SYSTEM
   ========================================== */
function searchProducts(keyword) {
    const filtered = window.db.getProducts().filter(p => p.name.toLowerCase().includes(keyword.toLowerCase()));
...
function filterStyle(styleName) {
    ...
    btns.forEach(b => {
        b.className = b.innerText === styleName ? "style-btn active px-4 py-1.5 rounded-full text-[11px] font-medium border btn-main text-white" : "style-btn px-4 py-1.5 rounded-full text-[11px] font-medium border bg-white text-gray-600";
    });
    renderStore();
}
...
function renderStoreCards(products) {
    ...
    if(products.length === 0) { cont.innerHTML = `<p class="col-span-2 text-center py-10 text-gray-400 text-xs">ไม่พบสินค้าในหมวดหมู่นี้</p>`; return; }
    cont.innerHTML = products.map((p) => {
        const realIdx = window.db.products.findIndex(item => item.name === p.name);
        return `
        <div class="product-card flex flex-col justify-between h-full">
...
function openProductDetail(idx) {
    ...
    detail.innerHTML = `
        <div class="sticky top-0 bg-white px-4 py-4 flex items-center justify-between border-b z-10 shadow-sm">
            ...
            <button onclick="openCartPage()" class="text-main text-lg"><i class="fa-solid fa-basket-shopping"></i></button>
        </div>
...
function revertTheme() { if (originalThemeBeforePreset) { window.db.config.theme = JSON.parse(JSON.stringify(originalThemeBeforePreset)); originalThemeBeforePreset = null; applyTheme(); window.db.saveConfig(window.db.config); renderAdminDashboard(); } }
...
function removePreset(i) { myConfirm("ต้องการลบพรีเซ็ตสีนี้ใช่ไหม?", () => { window.db.config.presets.splice(i, 1); window.db.saveConfig(window.db.config); renderPresets(); }); }

/* ==========================================
   5. USER ACCOUNT WALL & HISTORIES
   ========================================== */
function openUserMenuPage() { hideAllPages(); document.getElementById('userMenuPage').classList.remove('hidden'); updateCreditDisplay(); switchHistoryTab('order'); }
function closeUserMenuPage() { closeSubPage('userMenuPage'); }
...
function submitTopup() {
    const u = window.db.getCurrentUser(); if(!u) return alert("กรุณาเข้าสู่ระบบสมาชิกก่อนทำรายการค่ะ");
    const amt = Number(document.getElementById('topupAmount').value); if(!amt || amt <= 0) return alert("กรุณากรอกจำนวนเงินโอนที่ถูกต้อง");
    
    myConfirm(`ยืนยันการแจ้งโอนเงินจำนวน ฿${amt} ใช่หรือไม่?`, () => {
        u.topupHistory.unshift({ date: new Date().toLocaleString(), amount: amt, status: "รออนุมัติ (กรุณาส่งหลักฐานทางไลน์)" });
        
        // บันทึกซิงค์ข้อมูลผู้ใช้ปัจจุบันกลับเข้ากลุ่ม Members หลัก
        let members = window.db.getMembers();
        ...
        document.getElementById('topupAmount').value = ""; renderHistoryLogs();
        navigator.clipboard.writeText(`💸 แจ้งโอนเงินเติมเครดิตบัญชี: ${u.username} จำนวน ฿${amt}`); window.location.href = "https://line.me/ti/p/~@309ranuu";
    });
}
function renderHistoryLogs() {
    ...
    if(currentHistoryTab === 'order') {
        if(!u.orderHistory || u.orderHistory.length === 0) { cont.innerHTML = `<p class="text-center text-gray-400 py-6 text-xs">ไม่มีประวัติการสั่งซื้อ</p>`; return; }
        ...
        if(!u.topupHistory || u.topupHistory.length === 0) { cont.innerHTML = `<p class="text-center text-gray-400 py-6 text-xs">ไม่มีประวัติการเติมเงิน</p>`; return; }
...
function renderReviewsList() {
    document.getElementById('reviewsContainer').innerHTML = reviewsData.map(r => `
        <div class="custom-panel-card text-xs space-y-2 shadow-sm">
            <div class="flex justify-between items-center"><span class="font-bold text-main">${r.name}</span><span class="text-yellow-400 font-bold">${'★'.repeat(r.score)}</span></div>
            <p class="text-gray-600 font-medium">${r.text}</p><p class="text-[9px] text-gray-300">📅 ${r.date}</p>
        </div>`).join('');
}
function addNewReviewPrompt() {
    const name = prompt("ระบุชื่อของคุณ:"); const text = prompt("พิมพ์ข้อความรีวิว:");
    if(name && text) { reviewsData.unshift({ name: name, score: 5, date: "วันนี้", text: text, reply: "" }); localStorage.setItem('web_reviews', JSON.stringify(reviewsData)); renderReviewsList(); }
}

/* ==========================================
   7. ADMIN BACKEND (พาร์ท 4 เพิ่ม/ลบหมวดหมู่กลับมาแล้ว)
   ========================================== */
function toggleAdminModal(s) { document.getElementById('loginModal').classList.toggle('hidden', !s); }
function checkAdminPassword() { 
    if(document.getElementById('adminPasswordInput').value === window.db.config.adminPass) { toggleAdminModal(false); document.getElementById('adminPasswordInput').value = ""; renderAdminDashboard(); } else alert("รหัสผ่านไม่ถูกต้อง!"); 
}
    ...
    dash.innerHTML = `
        ...
        <div class="space-y-6 pb-24 text-xs">
            <div class="bg-gray-50 p-4 rounded-3xl border">
                ...
                <input type="text" id="cfgGasUrl" value="${cfg.googleAppsScriptUrl || ''}" class="w-full p-3 border rounded-xl mb-3" placeholder="URL ระบบดึงสิทธิ์ไดร์ฟ">
            ...
            <div class="bg-gray-50 p-4 rounded-3xl border">
                <h3 class="font-bold text-xs mb-3 text-main">2. ปรับแต่งโทนสี & พรีเซ็ต</h3>
                ...
                <div class="flex gap-2 mb-3">
                    <button id="savePresetBtn" onclick="savePreset()" class="flex-1 py-3 btn-main rounded-xl font-bold">${editingPresetIdx !== null ? 'ยืนยันบันทึกทับ' : 'บันทึกพรีเซ็ตสีใหม่'}</button>
                ...
                ${['Categories', 'SubCategories', 'Brands'].map(k => `
                    <div class="mb-3"><label class="text-[10px] font-bold text-gray-400 uppercase">${k}</label>
                        <div class="flex flex-wrap gap-1 mt-1">
                            ${tax[k.charAt(0).toLowerCase() + k.slice(1)].map((v, i) => `<span class="bg-white px-2.5 py-1.5 rounded-xl border border-main font-semibold">${v} <button onclick="removeTax('${k}', ${i})" class="text-red-400 font-bold ml-1">×</button></span>`).join('')}
...
function editProduct(idx) {
    ...
    document.getElementById('admDriveShare').checked = p.autoDriveShare || false; document.getElementById('admDriveFolderId').value = p.googleDriveFolderId || "";
...
function renderCart() {
    ...
    cont.innerHTML = cart.map((i, idx) => `
        <div class="card-bg p-3 rounded-2xl flex gap-3 items-center border shadow-sm text-xs">
            <img src="${i.img}" class="w-12 h-12 rounded-xl object-cover border"><div class="flex-1 font-bold text-main truncate">${i.name}</div>
            <div class="flex items-center gap-1.5"><button onclick="updateQty(${idx},-1)" class="w-7 h-7 border rounded-lg">-</button><span class="w-4 text-center font-bold">${i.qty}</span><button onclick="updateQty(${idx},1)" class="w-7 h-7 border rounded-lg">+</button></div>
...
function finalizeOrder() {
    // ดึงสถานะตรวจสอบการล็อกอินก่อนชำระเงิน
    const u = window.db.getCurrentUser();
    if(!u) { alert("กรุณาทำเข้าสู่ระบบสมาชิกก่อนกดสรุปยอดสั่งซื้อ เพื่อให้สิทธิ์เครดิตทำงานได้อย่างถูกต้องค่ะ"); toggleUserAuthModal(true); return; }
    ...
    rec.innerHTML = `<div class="p-4 max-w-[500px] mx-auto text-xs space-y-4">
        ...
        <div class="bg-white p-5 rounded-3xl border space-y-3">
            ...
            ${hasFontOrDeco ? `<input type="email" id="cusEmail" placeholder="ระบุ Gmail สำหรับร่วมสิทธิ์ไดร์ฟ *" class="w-full p-3 border rounded-xl outline-none">` : ''}
...
function processOrderPayment(method) {
    ...
    let total = cart.reduce((s, i) => s + (i.price-i.discount)*i.qty, 0); 
    const u = window.db.getCurrentUser(); 
    if(!u) return alert("บัญชีสมาชิกหลุดการเชื่อมต่อ");

    if(method === 'credit') {
        if(u.credit < total) return alert("เครดิตสะสมคงเหลือของคุณไม่เพียงพอค่ะ กรุณาเติมเงินก่อนนะคะ");
        u.credit -= total;
    }

    // บันทึกประวัติลงประวัติออเดอร์ของสมาชิก
    if(!u.orderHistory) u.orderHistory = [];
    u.orderHistory.unshift({ date: new Date().toLocaleString(), items: cart.map(i => `${i.name} (x${i.qty})`).join(', '), total: total, status: "สำเร็จ" });
    
    // บันทึกข้อมูลอัปเดตกลับฐานข้อมูลกลุ่มหลัก
    let members = window.db.getMembers();
    const idx = members.findIndex(m => m.username === u.username);
    if(idx !== -1) members[idx] = u;
    window.db.saveMembers(members); window.db.saveCurrentUser(u);
    ...
    cart.forEach(item => {
        if(item.autoDriveShare && item.googleDriveFolderId && emailVal !== "") {
            if(window.db.config.googleAppsScriptUrl) {
                fetch(window.db.config.googleAppsScriptUrl, { method: "POST", mode: "no-cors", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: emailVal, folderId: item.googleDriveFolderId }) });
            }
        }
    });
...
/* ==========================================
   9. CORE LAYOUT CONTROLLER
   ========================================== */
function hideAllPages() {
    ['mainPage', 'topSearchBar', 'mainHeader', 'floatingBottomNav', 'productDetailPage', 'cartPage', 'receiptPage', 'userMenuPage', 'reviewPage', 'adminDashboard'].forEach(id => {
...
function deleteProduct(idx) { myConfirm("ต้องการลบสินค้านี้?", () => { window.db.products.splice(idx,1); window.db.saveProducts(window.db.products); renderAdminDashboard(); }); }
function renderCategoryFilter() {
    const cont = document.getElementById('categoriesContainer'); const tax = window.db.getTaxonomy(); if(!cont) return;
    cont.innerHTML = `<button onclick="storeFilterCat='ทั้งหมด'; renderStore();" class="px-4 py-1.5 rounded-full text-[11px] font-bold border btn-main text-white">ทั้งหมด</button>` +
...
function openPromotionModal() { document.getElementById('promotionModal').classList.remove('hidden'); }
